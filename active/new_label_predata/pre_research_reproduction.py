...
def calculate_metrics_from_confusion(TP: int, TN: int, FP: int, FN: int) -> dict[str, float]:
    total = TP + TN + FP + FN
...
def update_confusion_counts(counts: dict[str, int], true_labels, predictions) -> None:
...
def format_param_label(params: dict[str, object]) -> str:
    return ",".join(f"{key}={params[key]}" for key in sorted(params))


def select_best_params_by_inner_confusion(
    ...
    """inner foldのTP/TN/FP/FNを合算し、合算F1が最大のSVM設定を選ぶ。"""
    best_params = None
    best_score = None
    best_label = None
    ...
    for params in ParameterGrid(param_grid):
        ...
        for inner_train_indices, inner_test_indices in inner_cv.split(
            train_documents,
            train_labels,
        ):
            inner_train_documents = [
                train_documents[int(index)] for index in inner_train_indices
            ]
            inner_train_labels = [
                train_labels[int(index)] for index in inner_train_indices
            ]
            inner_test_documents = [
                train_documents[int(index)] for index in inner_test_indices
            ]
            inner_test_labels = [
                train_labels[int(index)] for index in inner_test_indices
            ]

            pipeline = build_svm_pipeline(memory=cache_dir)
            pipeline.set_params(**params)
            pipeline.fit(inner_train_documents, inner_train_labels)
            predictions = pipeline.predict(inner_test_documents)
            update_confusion_counts(counts, inner_test_labels, predictions)

        score = calculate_metrics_from_confusion(**counts)["f1"]
        param_label = format_param_label(params)
        if (
            best_params is None
            or score > best_score
            or (score == best_score and param_label < best_label)
        ):
            best_params = dict(params)
            best_score = score
            best_label = param_label

    if best_params is None:
        raise RuntimeError("No SVM parameter candidates were evaluated.")
    return best_params
...
def evaluate_outer_fold(
    ...
    with tempfile.TemporaryDirectory(
        ...
        best_params = select_best_params_by_inner_confusion(
            train_documents,
            train_labels,
            inner_cv,
            param_grid,
        ...
        best_estimator = build_svm_pipeline(memory=cache_dir)
        best_estimator.set_params(**best_params)
        best_estimator.fit(train_documents, train_labels)
        predictions = best_estimator.predict(test_documents)

    feature_count = len(
        best_estimator.named_steps["tfidf"].feature_names_
    )
    return {
...
def evaluate_svm_prior_research(documents: list[str], labels: list[int]) -> dict[str, object]:
    ...
    update_confusion_counts(counts, labels, predictions)
    TP = counts["TP"]
    TN = counts["TN"]
    FP = counts["FP"]
    FN = counts["FN"]
    metrics = calculate_metrics_from_confusion(TP, TN, FP, FN)

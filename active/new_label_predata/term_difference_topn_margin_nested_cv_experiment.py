...
DEFAULT_FEATURE_COUNT_WORKER_LIMIT = 10
...
def load_corpus():
    ...
    documents_1, documents_0 = load_documents(
        ...
        dataset_names=PRE_RESEARCH_NEW_LABEL_DATASET_NAMES,
    )
...
def confusion_counts_from_predictions(true_labels, predictions):
...
def add_confusion_counts(total_counts, fold_counts):
    for metric in ("TP", "TN", "FP", "FN"):
        total_counts[metric] += int(fold_counts[metric])
...
def build_selected_feature_matrices(
    ...
    if requested_feature_mode in (None, COLUMN_MIN_MAX_FEATURE_MODE):
        ...
        feature_matrices["feature_metadata"][COLUMN_MIN_MAX_FEATURE_MODE] = {
                "selected_feature_names": selected_feature_names,
        ...
        feature_matrices[COLUMN_MIN_MAX_FEATURE_MODE] = {
            "train": X_train_min_max_features,
            "test": X_test_min_max_features,
        }
    if requested_feature_mode in (None, ROW_L2_FEATURE_MODE):
        ...
        feature_matrices["feature_metadata"][ROW_L2_FEATURE_MODE] = {
                "selected_feature_names": selected_feature_names,
                "feature_train_min": np.full(len(selected_terms), np.nan),
                "feature_train_max": np.full(len(selected_terms), np.nan),
                "feature_transform": np.full(
        ...
        feature_matrices[ROW_L2_FEATURE_MODE] = {
...
def _build_vocabulary_variant(
    ...
    if RELATIVE_TERM_FREQUENCY_MODE in SELECTION_MODES:
        variant["term_frequency_df"] = sort_difference_table(
    ...
    if LOG_TERM_FREQUENCY_MODE in SELECTION_MODES:
        variant["log_term_frequency_df"] = sort_difference_table(
    ...
    if TF_S_MODE in SELECTION_MODES:
        variant["tf_s_df"] = sort_difference_table(
    ...
    if LOG_TF_S_MODE in SELECTION_MODES:
        variant["log_tf_s_df"] = sort_difference_table(
...
class SplitFeatureContext:
    ...
    def __init__(self, train_docs, train_labels, test_docs, test_labels, train_doc_ids, test_doc_ids):
        ...
        if any(_uses_document_frequency_filter(mode) for _, mode in FEATURE_CONDITIONS):
            (
            ...
            self.vocabulary_variants[True] = filtered_variant
    ...
    def get_feature_bundle(self, selection_mode, feature_count, feature_mode):
        ...
        if cache_key not in self.feature_cache:
            ...
            feature_matrices = build_selected_feature_matrices(
                ...
                self.train_word_counts,
                self.test_word_counts,
                requested_feature_mode=base_feature_mode,
            )
...
def fit_and_evaluate_split_context(
    ...
    counts = confusion_counts_from_predictions(split_context.test_labels, y_pred)
    result = {
...
def evaluate_candidate_across_outer_folds(
    ...
    for fold_bundle in outer_fold_contexts:
        inner_counts = {"TP": 0, "TN": 0, "FP": 0, "FN": 0}
        for inner_fold, inner_context in enumerate(fold_bundle["inner_contexts"], start=1):
            ...
            if inner_result is None:
                return None
            add_confusion_counts(inner_counts, inner_result)

        inner_score = calculate_metrics_from_confusion(
            inner_counts["TP"],
            inner_counts["TN"],
            inner_counts["FP"],
            inner_counts["FN"],
        ...
        fold_records.append(
            {
                ...
                # inner foldのTP/TN/FP/FNを合算したF1を、候補選択基準として使う。
...
def run_nested_cv_for_condition(
    ...
    for record in selected_records:
        add_confusion_counts(total_counts, record)
    result.update(total_counts)
    result.update(
        calculate_metrics_from_confusion(
            result["TP"],
            result["TN"],
...
def save_nested_cv_result_csvs(result, output_dir, output_prefix):
    ...
    # 条件全体の合算混同行列ベースの指標を 1 行で見られるよう、summary 用 CSV を先に作る。
    metrics_row = {
        ...
        "FP": result.get("FP", ""),
        "FN": result.get("FN", ""),
        "accuracy": round_half_up(result.get("accuracy", "")),
        "recall": round_half_up(result.get("recall", "")),
        "precision": round_half_up(result.get("precision", "")),
        "f1": round_half_up(result.get("f1", "")),
    ...
    # fold ごとに実際に採用された候補と混同行列も別 CSV へ保存する。
    ...
    if not selected_records_df.empty:
        ...
        selected_records_df["inner_score"] = selected_records_df["inner_score"].map(round_half_up)
        ...
        row.update(
            {
                ...
                "FP": result["FP"],
                "FN": result["FN"],
                "accuracy": round_half_up(result["accuracy"]),
                "recall": round_half_up(result["recall"]),
                "precision": round_half_up(result["precision"]),
                "f1": round_half_up(result["f1"]),
                "best_cs": ",".join(map(str, result.get("best_cs", []))),
                "best_gammas": ",".join(map(str, result.get("best_gammas", []))) if "best_gammas" in result else "",
            }
        )
    ...
    for condition_result in feature_count_job_result["condition_results"]:
        ...
        if result.get("selected_records"):
            ...
            if (
                ...
                or result["f1"] > current_best["raw_f1"]
                or (
                    result["f1"] == current_best["raw_f1"]

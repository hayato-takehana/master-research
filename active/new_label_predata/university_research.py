...
def add_confusion_counts(total_counts: dict[str, int], fold_counts: dict[str, int]) -> None:
...
def fit_and_evaluate_context(
    ...
    metrics = calculate_metrics_from_confusion(int(tp), int(tn), int(fp), int(fn))
    return {
...
def evaluate_candidate_across_outer_folds(
    ...
    for fold_bundle in outer_fold_contexts:
        inner_counts = empty_confusion_counts()
...
def aggregate_selected_records(
    ...
    total_counts = empty_confusion_counts()

...
MAX_DEPTH_VALUES = [2, 3, 4, 6, 8]
LEARNING_RATE_VALUES = [0.01, 0.05, 0.1, 0.2]
MIN_CHILD_WEIGHT_VALUES = [0, 1, 3, 5]
...
def build_full_corpus_tfidf_matrix(documents):
    """テストデータも含む全文書でTF-IDFをfitし、全体行列を返す。"""
    tf_idf = Tf_idf(
        list(documents),
        True,
        MIN_LEN,
...
class SplitTfidfContext:
    """1つのsplitについて、全文書fit済みTF-IDF行列のtrain/test部分を保持する。"""
    ...
    def __init__(
        ...
        self.train_labels = np.asarray(labels[train_indices], dtype=int)
        self.test_labels = np.asarray(labels[test_indices], dtype=int)
        self.X_train = X_all[train_indices]
        self.X_test = X_all[test_indices]
    ...
    for inner_train_index, inner_test_index in inner_cv.split(
        ...
        inner_contexts.append(
            SplitTfidfContext(
                X_all,
                labels,
                outer_train_index[inner_train_index],
                outer_train_index[inner_test_index],
                feature_count,
            )
        )
...
_SPLIT_WORKER_X_ALL = None
_SPLIT_WORKER_FEATURE_COUNT = None


def _init_split_context_worker(documents, labels, X_all, feature_count):
    """各常駐ワーカーへcorpusを一度だけ渡す。"""
    global _SPLIT_WORKER_DOCUMENTS
    global _SPLIT_WORKER_LABELS
    global _SPLIT_WORKER_X_ALL
    global _SPLIT_WORKER_FEATURE_COUNT

    _SPLIT_WORKER_DOCUMENTS = documents
    _SPLIT_WORKER_LABELS = labels
    _SPLIT_WORKER_X_ALL = X_all
    _SPLIT_WORKER_FEATURE_COUNT = feature_count


def _build_outer_fold_context_worker(fold_payload):
    if (
        _SPLIT_WORKER_DOCUMENTS is None
        or _SPLIT_WORKER_LABELS is None
        or _SPLIT_WORKER_X_ALL is None
    ):
...
def build_outer_fold_contexts(
    ...
    X_all, feature_names, _ = build_full_corpus_tfidf_matrix(documents)
    ...
    if worker_count == 1:
        ...
        executor = futures.ProcessPoolExecutor(
            ...
            initargs=(documents, labels, X_all, feature_count),
        )
        try:
            future_to_fold = {
...
def build_xgboost_model(n_estimators, max_depth, learning_rate, min_child_weight, y_train):
...
def compute_signed_margins(model, X_train, y_train):
    decision_values = np.asarray(model.predict(X_train, output_margin=True)).ravel()
...
def format_param_label(c, gamma=None):
    ...
    return (
        f"n_estimators={c}, max_depth={max_depth}, "
...
def print_result(result, saved_paths):
    ...
    if not result.get("selected_records"):
        ...
        print(
            "selected (max_depth, learning_rate, min_child_weight) values: "
            f"{result.get('best_gammas', [])}"
        )
        print(

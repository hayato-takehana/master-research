...
def build_lightgbm_model(n_estimators, num_leaves, max_depth, learning_rate):
...
def compute_signed_margins(model, X_train, y_train):
    decision_values = np.asarray(model.predict(X_train, raw_score=True)).ravel()
...
def fit_and_evaluate_context(
    ...
    num_leaves, max_depth, learning_rate = gamma
    model = build_lightgbm_model(c, num_leaves, max_depth, learning_rate)

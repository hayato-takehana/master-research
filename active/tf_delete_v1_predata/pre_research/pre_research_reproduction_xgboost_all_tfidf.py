...
N_SPLITS = 10
RANDOM_STATE = 42
RESULT_CSV = SAVE_DIR / "pre_research_xgboost_reproduction_all_tfidf_results.csv"
...
def build_xgboost_pipeline(memory=None) -> Pipeline:
    return Pipeline(
        steps=[
            (
                "tfidf",
...
def evaluate_outer_fold(
    ...
    grid_search.fit(X_train, train_labels, sample_weight=sample_weight)
...
def run_outer_folds(
    ...
    if worker_count == 1:
        ...
        executor = futures.ProcessPoolExecutor(
            ...
            initargs=(documents, labels, N_SPLITS, XGBOOST_PARAM_GRID),
        )
        try:
            future_to_fold = {
...
def evaluate_xgboost_prior_research(documents: list[str], labels: list[int]) -> dict[str, object]:
...
def main() -> int:
    ...
    result = evaluate_xgboost_prior_research(documents, labels)

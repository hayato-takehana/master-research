...
N_SPLITS = 10
RANDOM_STATE = 42
RESULT_CSV = SAVE_DIR / "pre_research_lightgbm_reproduction_all_tfidf_results.csv"
...
def build_full_corpus_tfidf_matrix(documents: list[str]):
...
def build_lightgbm_pipeline(memory=None) -> Pipeline:
    return Pipeline(
        steps=[
            (
                "tfidf",
...
def build_lightgbm_classifier() -> LGBMClassifier:
    """先行研究の付録表B.1のLightGBMグリッドを使う分類器。"""
    return LGBMClassifier(class_weight="balanced", random_state=RANDOM_STATE)
...
def evaluate_outer_fold(
    ...
    grid_search = GridSearchCV(
        estimator=build_lightgbm_classifier(),
...
def run_outer_folds(
    ...
    if worker_count == 1:
        ...
        executor = futures.ProcessPoolExecutor(
            ...
            initargs=(documents, labels, N_SPLITS, LIGHTGBM_PARAM_GRID),
        )
        try:
            future_to_fold = {
...
def evaluate_lightgbm_prior_research(documents: list[str], labels: list[int]) -> dict[str, object]:
...
def main() -> int:
    ...
    result = evaluate_lightgbm_prior_research(documents, labels)

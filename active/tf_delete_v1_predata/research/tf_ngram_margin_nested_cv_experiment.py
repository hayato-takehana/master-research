...
def read_positive_int_env(env_name, default_value):
    """正の整数環境変数を読み取る。無効値はデフォルトへ戻す。"""
    env_value = os.environ.get(env_name, "").strip()
    if not env_value:
        return default_value
    try:
...
def read_bool_env(env_name, default_value=False):
    """真偽値環境変数を読み取る。"""
...
SAVE_DIR = (
    ...
    / "tf_ngram_margin_nested_cv_experiment"
)
...
MAX_CACHED_FEATURE_COUNT = max(FEATURE_COUNT_CANDIDATES)
SAVE_PER_FEATURE_COUNT_OUTPUTS = read_bool_env(
    "TERM_DIFFERENCE_SAVE_PER_FEATURE_OUTPUTS",
    False,
)
SAVE_DETAILED_OUTPUTS = read_bool_env(
    "TERM_DIFFERENCE_SAVE_DETAILED_OUTPUTS",
    False,
)
NGRAM_RANGES = (
    (1, 2),
...
USE_CANDIDATE_WORKER = read_bool_env(
    "TERM_DIFFERENCE_USE_CANDIDATE_WORKER",
    False,
)
CANDIDATE_THREAD_WORKERS_ENV = "TERM_DIFFERENCE_CANDIDATE_THREADS"
DEFAULT_CANDIDATE_THREAD_WORKER_LIMIT = read_positive_int_env(
    CANDIDATE_THREAD_WORKERS_ENV,
    20,
)
N_SPLITS = 10
RANDOM_STATE = 42
...
DEFAULT_SPLIT_CONTEXT_WORKER_LIMIT = 5
FEATURE_COUNT_WORKERS_ENV = "TERM_DIFFERENCE_FEATURE_COUNT_WORKERS"
DEFAULT_FEATURE_COUNT_WORKER_LIMIT = 1
...
def normalize_ngram_range(ngram_range):
    """ngram_range を `(min_n, max_n)` の整数タプルへ正規化する。"""
    if len(ngram_range) != 2:
        raise ValueError(f"ngram_range must contain two values: {ngram_range}")
    min_n, max_n = (int(value) for value in ngram_range)
    if min_n <= 0 or max_n < min_n:
        raise ValueError(f"Invalid ngram_range: {ngram_range}")
    return min_n, max_n


def format_ngram_range_value(ngram_range):
    """CSVに入れる n-gram 条件名を返す。"""
    min_n, max_n = normalize_ngram_range(ngram_range)
    if min_n == max_n:
        return str(min_n)
    return f"{min_n}-{max_n}"


def format_ngram_range_label(ngram_range):
    """ログ表示向けの n-gram 条件名を返す。"""
    min_n, max_n = normalize_ngram_range(ngram_range)
    if min_n == max_n:
        return f"{min_n}語のみ"
    return f"{min_n}～{max_n}語"


def format_ngram_range_slug(ngram_range):
    """n-gram 条件をディレクトリ名へ変換する。"""
    min_n, max_n = normalize_ngram_range(ngram_range)
    if min_n == max_n:
        return f"ngram_{min_n}"
    return f"ngram_{min_n}_{max_n}"


def vectorize_train_test_documents(train_docs, test_docs, ngram_range):
    """train 側で語彙を学習し、その語彙で train/test の出現回数行列を作る。

    ここで必ず train 側だけで vectorizer を fit することで、test 側の語彙が
    先に混ざる情報リークを防ぐ。
    """
    ngram_range = normalize_ngram_range(ngram_range)
...
def build_relative_min_max_feature_matrices(
    X_train_count,
    X_test_count,
    count_feature_names,
    selected_terms_df,
    ...
    """相対TF上位語について、列Min-Max特徴だけを作る。"""
    selected_terms = selected_terms_df["term"].tolist()
    count_vocab_index = {term: idx for idx, term in enumerate(count_feature_names)}
    term_indices = [count_vocab_index[term] for term in selected_terms]

    X_train_source = _relative_frequency_matrix(
        X_train_count,
        train_word_counts,
    )
    X_test_source = _relative_frequency_matrix(
        X_test_count,
        test_word_counts,
    )
    train_values = X_train_source[:, term_indices].toarray()
    test_values = X_test_source[:, term_indices].toarray()
    (
        X_train_features,
        X_test_features,
        train_mins,
        train_maxs,
    ) = _min_max_scale_columns_from_train(train_values, test_values)

    selected_feature_names = [f"{term}__tf" for term in selected_terms]
    return {
        "selected_terms": selected_terms,
        "feature_metadata": {
            COLUMN_MIN_MAX_FEATURE_MODE: {
                ...
                "feature_transform": np.full(
                    len(selected_terms),
                    "relative_frequency_column_min_max",
                    dtype=object,
                ),
            },
        },
...
def _slice_feature_value(value, feature_count):
...
def slice_feature_matrices(feature_matrices, feature_count):
    """最大語数で作った特徴行列から、先頭 feature_count 語だけを返す。"""
    sliced_matrices = {
        "selected_terms": feature_matrices["selected_terms"][:feature_count],
        "feature_metadata": {},
    }
    for mode, metadata in feature_matrices["feature_metadata"].items():
        sliced_matrices["feature_metadata"][mode] = {
            key: _slice_feature_value(value, feature_count)
            for key, value in metadata.items()
        }
    for mode, matrices in feature_matrices.items():
        if mode in ("selected_terms", "feature_metadata"):
            continue
        sliced_matrices[mode] = {
            "train": matrices["train"][:, :feature_count],
            "test": matrices["test"][:, :feature_count],
        }
...
class SplitFeatureContext:
    ...
    def __init__(
        ...
        """split の学習データだけで語彙とTF差分表を作る。"""
        self.ngram_range = normalize_ngram_range(ngram_range)
    ...
    def get_feature_bundle(self, selection_mode, feature_count, feature_mode):
        """最大語数で作った特徴行列を使い、指定Nのprefixだけを返す。"""
        filter_applied = _uses_document_frequency_filter(feature_mode)
        base_feature_mode = _base_feature_mode(feature_mode)
        if filter_applied:
            raise ValueError(
                "Document-frequency filtered feature modes are disabled in "
                "the n-gram accuracy-only experiment."
            )
        if (
            selection_mode != RELATIVE_TERM_FREQUENCY_MODE
            or base_feature_mode != COLUMN_MIN_MAX_FEATURE_MODE
        ):
            raise ValueError(
                "Only relative term-frequency selection with column Min-Max "
                "features is enabled in this n-gram accuracy-only experiment."
            )
        cache_feature_count = max(feature_count, MAX_CACHED_FEATURE_COUNT)
        cache_key = (selection_mode, cache_feature_count, filter_applied)
        if cache_key not in self.feature_cache:
            ...
            feature_matrices = build_relative_min_max_feature_matrices(
            ...
            self.feature_cache[cache_key] = {
                ...
                "feature_count": cache_feature_count,
        ...
        cached_bundle = self.feature_cache[cache_key]
        if base_feature_mode not in cached_bundle["feature_matrices"]:
            raise ValueError(
                "Feature mode is incompatible with selection mode: "
                f"selection_mode={selection_mode}, feature_mode={feature_mode}"
            )
        sliced_terms_df = (
            cached_bundle["selected_terms_df"]
            .head(feature_count)
            .copy()
            .reset_index(drop=True)
        )
        sliced_feature_matrices = slice_feature_matrices(
            cached_bundle["feature_matrices"],
            feature_count,
        )
        return {
            **cached_bundle,
            "feature_count": feature_count,
            "selected_terms_df": sliced_terms_df,
            "selected_term_count": len(sliced_feature_matrices["selected_terms"]),
            "feature_matrices": sliced_feature_matrices,
        }
...
def build_outer_fold_context_from_indices(
    ...
    log_progress(
        ...
        f"preparing train/test feature contexts ({format_ngram_range_label(ngram_range)})"
    )
...
_SPLIT_WORKER_NGRAM_RANGE = None


def _init_split_context_worker(documents, labels, doc_ids, ngram_range):
    """各ワーカーへcorpusを1回だけ渡す。"""
    global _SPLIT_WORKER_DOCUMENTS
    global _SPLIT_WORKER_LABELS
    global _SPLIT_WORKER_DOC_IDS
    global _SPLIT_WORKER_NGRAM_RANGE

    _SPLIT_WORKER_DOCUMENTS = documents
    _SPLIT_WORKER_LABELS = labels
    _SPLIT_WORKER_DOC_IDS = doc_ids
    _SPLIT_WORKER_NGRAM_RANGE = normalize_ngram_range(ngram_range)
...
def _build_outer_fold_context_worker(fold_payload):
    ...
    return build_outer_fold_context_from_indices(
        ...
        _SPLIT_WORKER_NGRAM_RANGE,
    )


def build_outer_fold_contexts(documents, labels, doc_ids, ngram_range):
    ...
    log_progress(
        f"[split setup] building outer/inner feature contexts for {N_SPLITS}-fold "
        f"nested CV ({format_ngram_range_label(ngram_range)})"
    )
        ...
        fold_records.append(
            {
                ...
                "ngram_range": format_ngram_range_value(fold_bundle["ngram_range"]),
                "selection_mode": selection_mode,
                "feature_count": feature_count,
...
def build_empty_result(selection_mode, feature_count, feature_mode, kernel, ngram_range):
    """候補が全滅した場合にも同じ形式で扱える空の結果辞書を作る。"""
    result = {
        "ngram_range": format_ngram_range_value(ngram_range),
        "selection_mode": selection_mode,
        "feature_count": feature_count,
...
def run_nested_cv_for_condition(
    ...
    ngram_range = outer_fold_contexts[0]["ngram_range"]
    ...
    condition_label = (
        f"{format_ngram_range_label(ngram_range)} / "
    ...
    def record_candidate_result(c, gamma, candidate_result):
    ...
    def evaluate_candidate_in_current_process(c, gamma):
        return {
            "status": "ok",
            "candidate_records": evaluate_candidate_across_outer_folds(
                outer_fold_contexts,
    ...
    if USE_CANDIDATE_WORKER:
        ...
        try:
            for c, gamma in candidate_pairs:
                record_candidate_result(c, gamma, candidate_worker.evaluate(c, gamma))
        finally:
            candidate_worker.close()
    else:
        candidate_thread_count = resolve_worker_count(
            CANDIDATE_THREAD_WORKERS_ENV,
            DEFAULT_CANDIDATE_THREAD_WORKER_LIMIT,
            total_candidates,
        )
        log_progress(
            "[condition setup] candidate worker disabled; evaluating candidates "
            f"with {candidate_thread_count} threads "
            f"(override with {CANDIDATE_THREAD_WORKERS_ENV})"
        )
        if candidate_thread_count == 1:
            for c, gamma in candidate_pairs:
                record_candidate_result(
                    c,
                    gamma,
                    evaluate_candidate_in_current_process(c, gamma),
                )
        else:
            with futures.ThreadPoolExecutor(max_workers=candidate_thread_count) as executor:
                future_to_candidate = {
                    executor.submit(
                        evaluate_candidate_in_current_process,
                        c,
                        gamma,
                    ): (c, gamma)
                    for c, gamma in candidate_pairs
                ...
                for future in futures.as_completed(future_to_candidate):
                    c, gamma = future_to_candidate[future]
                    record_candidate_result(c, gamma, future.result())
    ...
    result["misclassified_counts"] = [
        record.get("misclassified_count", "")
...
def save_nested_cv_result_csvs(result, output_dir, output_prefix):
    ...
    param_status_rows.extend(
        ...
        for label in result.get("timeout_param_labels", [])
    )
    pd.DataFrame(
        param_status_rows,
        columns=["ngram_range", "status", "param_label"],
    ).to_csv(
        params_path,
        index=False,
        encoding="utf-8-sig",
...
def save_outer_fold_selected_terms(result, outer_fold_contexts, output_dir):
    ...
    for record in result.get("selected_records", []):
        ...
        selected_terms_df["ngram_range"] = result["ngram_range"]
        ...
        if record["selection_mode"] == RELATIVE_TERM_FREQUENCY_MODE:
            ...
            for iqr_outlier_row in iqr_outlier_rows:
                iqr_outlier_row["ngram_range"] = result["ngram_range"]
            iqr_outlier_path = (
...
def save_term_frequency_iqr_audit(
    ...
    for fold_bundle in outer_fold_contexts:
        outer_fold = fold_bundle["outer_fold"]
        ngram_range_value = format_ngram_range_value(fold_bundle["ngram_range"])
        ...
        fold_terms_df.insert(1, "ngram_range", ngram_range_value)
        fold_terms_df.insert(2, "audit_feature_count", maximum_feature_count)
        ...
        fold_outlier_rows = build_term_frequency_iqr_outlier_rows(
            fold_terms_df,
            outer_context.X_train_count,
            outer_context.train_labels,
            outer_context.train_doc_ids,
        ...
        for fold_outlier_row in fold_outlier_rows:
            fold_outlier_row["ngram_range"] = ngram_range_value
        all_outlier_rows.extend(fold_outlier_rows)
    ...
    for selection_mode, feature_mode in FEATURE_CONDITIONS:
        for feature_count in feature_count_candidates:
            ...
            summary_rows.append(
                {
                    "selection_mode": selection_mode,
                    "ngram_range": format_ngram_range_value(
                        outer_fold_contexts[0]["ngram_range"]
                    ),
                    "feature_mode": feature_mode,
                    "feature_count": feature_count,
...
def precompute_max_feature_bundles(outer_fold_contexts):
    """各splitで最大語数の特徴行列を一度だけ作り、以降はprefix sliceで使う。"""
    start = time.perf_counter()
    total_contexts = sum(
        1 + len(fold_bundle["inner_contexts"])
        for fold_bundle in outer_fold_contexts
    )
    completed_contexts = 0
    log_progress(
        f"[feature cache] precomputing top-{MAX_CACHED_FEATURE_COUNT} "
        f"feature matrices for {total_contexts} split contexts"
    )
    for fold_bundle in outer_fold_contexts:
        contexts = [
            fold_bundle["outer_context"],
            *fold_bundle["inner_contexts"],
        ]
        for context in contexts:
            context.get_feature_bundle(
                RELATIVE_TERM_FREQUENCY_MODE,
                MAX_CACHED_FEATURE_COUNT,
                COLUMN_MIN_MAX_FEATURE_MODE,
            )
            completed_contexts += 1
        log_progress(
            f"[feature cache] outer fold {fold_bundle['outer_fold']}/{N_SPLITS} "
            f"cached ({completed_contexts}/{total_contexts} contexts)"
        )
    log_progress(
        f"[feature cache] completed in "
        f"{format_elapsed_seconds(time.perf_counter() - start)}"
    )
...
def run_feature_count_job(feature_count, outer_fold_contexts, experiment_conditions):
    """1つの特徴数について指定された特徴条件とカーネルを評価する。"""
    ngram_label = format_ngram_range_label(outer_fold_contexts[0]["ngram_range"])
    log_progress(
        f"\n[feature count start] {ngram_label} "
        f"feature_count={feature_count} pid={os.getpid()}"
    )
    condition_results = []
...
def record_feature_count_job_result(
    ...
    feature_count_root = Path(output_root) / format_feature_count_slug(feature_count)
    ...
    for condition_result in feature_count_job_result["condition_results"]:
        ...
        condition_dir = None
        if SAVE_PER_FEATURE_COUNT_OUTPUTS:
            condition_dir = get_condition_metrics_dir(
        ...
        if SAVE_PER_FEATURE_COUNT_OUTPUTS:
            ...
            print_brief_result(
                result,
                format_condition_label(selection_mode, feature_mode, kernel),
            )
        if SAVE_DETAILED_OUTPUTS:
...
def print_result(result, heading):
    ...
    print(f"ngram range: {result['ngram_range']}")
    ...
    print(
        "param counts: "
        f"valid={len(result.get('valid_param_labels', []))}, "
        f"dropped={len(result.get('dropped_param_labels', []))}, "
        f"timeout={len(result.get('timeout_param_labels', []))}"
    )
    if "saved_csv_paths" in result:
        print(f"saved metrics csv: {result['saved_csv_paths']['metrics']}")
        print(f"saved selected-records csv: {result['saved_csv_paths']['selected_records']}")
...
def print_brief_result(result, condition_label):
    """精度確認用に、1条件ぶんの結果を短く表示する。"""
    prefix = (
        f"[result] ngram={result['ngram_range']} "
        f"feature_count={result['feature_count']} "
        f"{condition_label}"
    )
    if not result.get("selected_records"):
        log_progress(
            f"{prefix}: no valid params "
            f"(valid={len(result.get('valid_param_labels', []))}, "
            f"dropped={len(result.get('dropped_param_labels', []))}, "
            f"timeout={len(result.get('timeout_param_labels', []))})"
        )
    ...
    log_progress(
        ...
        f"valid_params={len(result.get('valid_param_labels', []))}"
    )


def run_ngram_experiment(documents, labels, doc_ids, pdf_names, ngram_range):
    """指定した n-gram 条件について、全特徴数の nested CV と出力保存を行う。"""
    ngram_range = normalize_ngram_range(ngram_range)
    ngram_label = format_ngram_range_label(ngram_range)
    ngram_output_dir = SAVE_DIR / format_ngram_range_slug(ngram_range)
    ngram_output_dir.mkdir(parents=True, exist_ok=True)

    global_margin_source_results = []
    log_progress(f"\n[main] building split contexts for {ngram_label}")
    ...
    precompute_max_feature_bundles(outer_fold_contexts)
    ...
    if feature_count_worker_count > 1:
        log_progress(
            "[parallel warning] feature-count workers copy the precomputed "
            "top-N feature cache to each process; this can be slower on Windows."
        )
    ...
    if SAVE_DETAILED_OUTPUTS:
        feature_count_paths = save_feature_count_term_outputs(
            outer_fold_contexts,
            FEATURE_COUNT_CANDIDATES,
            ngram_output_dir / FEATURE_COUNT_DIR_NAME,
        )
        log_progress(
        ...
        tf_iqr_paths = save_term_frequency_iqr_audit(
            outer_fold_contexts,
            FEATURE_COUNT_CANDIDATES,
            ngram_output_dir / FEATURE_COUNT_DIR_NAME / "tf_iqr",
        )
        ...
        try:
            global_margin_csv_path = save_global_top_margin_misclassified_documents(
                ...
                outer_fold_contexts,
                pdf_names,
                ngram_output_dir / GLOBAL_TOP_MARGIN_MISCLASSIFIED_FILENAME,
        ...
        log_progress("[main] detailed term/misclassified/IQR outputs skipped")

    clear_feature_caches(outer_fold_contexts)

    return ngram_output_dir
...
if __name__ == "__main__":
    ...
    log_progress(
        "[main] defaults: "
        f"feature_counts={FEATURE_COUNT_CANDIDATES[0]}..{FEATURE_COUNT_CANDIDATES[-1]} "
        f"step={FEATURE_COUNT_CANDIDATES[1] - FEATURE_COUNT_CANDIDATES[0]}, "
        f"split_workers={DEFAULT_SPLIT_CONTEXT_WORKER_LIMIT}, "
        f"candidate_threads={DEFAULT_CANDIDATE_THREAD_WORKER_LIMIT}, "
        f"feature_count_workers={DEFAULT_FEATURE_COUNT_WORKER_LIMIT}, "
        f"save_per_feature_count_outputs={SAVE_PER_FEATURE_COUNT_OUTPUTS}"
    )
    output_dirs = []
    ...
    log_progress(
        "[main] completed n-gram experiments: "
        + ", ".join(str(output_dir) for output_dir in output_dirs)
    )
